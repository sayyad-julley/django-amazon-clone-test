...
**Objective**: 
You are a Senior Full-Stack Engineer. Complete the following SDLC workflow for Issue AGENTIC-150.
...
✓ No breaking changes
- Repo: django-amazon-clone-test (Branch: feature/agentic_150)
...
6. **QA Reporting**:
   ...
   - Issue ID: AGENTIC-150
   - Project ID: Get from 'mcp__sdlc-tools__get_project_context' if needed

7. **Delivery**: 
   - Commit changes (use git commit with descriptive message)
   - Push branch to remote (git push origin feature/agentic_150)
   - Use 'open_github_pr' tool to submit your work
   - PR title should reference the issue: "AGENTIC-150: Add "Refresh" button in admin dashboard home page"
...
- **DjangoEcommerceApp/templates/admin_templates/home.html**: title='Add Refresh Button' description='Implement refresh button in admin dashboard home template' priority=<Priority.P2: 'P2'> assignee=None due_date=None files=[FileReference(path='DjangoEcommerceAp...

### Pattern Analysis

- **DjangoEcommerceApp/templates/admin_templates/home.html**: title='Add Refresh Button to Admin Home' description='Implement a simple refresh button in admin dashboard home template' priority=<Priority.P2: 'P2'> assignee=None due_date=None files=[FileReference(...

### Dependency Check

- **dependency_check**: title='Proceed with Refresh Button Implementation' description='No dependency modifications required for AGENTIC-150' priority=<Priority.P2: 'P2'> assignee=None due_date=None files=[]...
