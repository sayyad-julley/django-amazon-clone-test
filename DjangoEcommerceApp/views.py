...
def adminRegistration(request):
    if request.method == 'POST':
        ...
        # In a real app, you'd use Django's UserCreationForm and handle validation
        user = User.objects.create_user(username=username, password=password)
        messages.success(request, "Registration Successful!")
        return HttpResponseRedirect(reverse("admin_login"))
    return render(request, "admin_templates/registration.html")
...
def adminPasswordReset(request):
    if request.method == 'POST':
        ...
        return HttpResponseRedirect(reverse("admin_login"))
    return render(request, "admin_templates/password_reset.html")