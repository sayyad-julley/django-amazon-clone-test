...
def pytest_runtest_makereport(item, call):
    ...
    # Report to Kualitee without nesting event loops
    try:
        try:
            # Case 1: we're already inside a running event loop (e.g. pytest-asyncio)
            loop = asyncio.get_running_loop()
            loop.create_task(_report_to_kualitee(test_case_id, status, evidence))
        except RuntimeError:
            # Case 2: no running loop; safe to create and run our own
            asyncio.run(_report_to_kualitee(test_case_id, status, evidence))
